...
def C_mechamical_mix(Cl_all,Ch_all,Cb_all, z_matrix,dz_matrix,ind_crop, level):
    ...
    num_of_pixle = len(ind_crop)
    layer_end = np.zeros(num_of_pixle,dtype=int)
    Cl_top_ave = np.zeros(num_of_pixle)
    Ch_top_ave = np.zeros(num_of_pixle)
    Cb_top_ave = np.zeros(num_of_pixle)
    
    
    for i in xrange(soil_layer_num-1):
        for n in xrange(num_of_pixle):
            j = ind_crop[n]
    ...
    for n in xrange(num_of_pixle):
        ...
        Cl_top_ave[n] = np.sum(dz_matrix[0:k+1,j]*Cl_all[0:k+1,j])/z_matrix[k,j]
        Ch_top_ave[n] = np.sum(dz_matrix[0:k+1,j]*Ch_all[0:k+1,j])/z_matrix[k,j]
        Cb_top_ave[n] = np.sum(dz_matrix[0:k+1,j]*Cb_all[0:k+1,j])/z_matrix[k,j]
    
    for n in xrange(num_of_pixle):
        j = ind_crop[n]
        k = layer_end[n]+1
        Cl_all[0:k,j]= Cl_top_ave[n]
        Ch_all[0:k,j]= Ch_top_ave[n]
        Cb_all[0:k,j]= Cb_top_ave[n]
