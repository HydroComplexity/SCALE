...
fig, ax = plt.subplots(1,1,figsize=(12,2.6), dpi=400 )  
#plt.yscale('log')
plt.rc("font", size=16)

for y in xrange(0,year):
    ax.bar(np.arange(d),rain[y*d:(y+1)*d],edgecolor ='grey',color="grey",alpha=0.07,label=r"rain fall intensity")

y_real = 2 # 0 is 2015, 1 is 2016, 2 is 2017
ax.bar(np.arange(d),rain[y_real*d:(y_real+1)*d],edgecolor ='royalblue',color="royalblue",label=r"rain fall intensity")
...
ax.set_xlim([0,365])
fig.tight_layout()
fig.savefig("rainfall_paper.pdf", bbox_inches="tight")
...
#    ax.bar(np.arange(month),rain_month[y*month:(y+1)*month],edgecolor ='grey',color="grey",alpha=0.1,label=r"rain fall intensity")
    ax.plot(np.arange(month),rain_month[y*month:(y+1)*month],color = 'royalblue',alpha=0.5)
...
ax.set_ylabel('Rainfall [mm/mo]', fontsize=16)
##ax.set_ylim([0,80])
ax.set_xlim([0,11])
ax.set_ylim([0,430])
fig.tight_layout()
