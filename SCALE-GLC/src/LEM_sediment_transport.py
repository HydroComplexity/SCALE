...
def Overland_SedimentTransport(wd_vector,slope,direction,Ca_vector,nrows,ncols,level,mann_vector):
#    print Ca_vector.shape
    ...
    mann = mann_vector.reshape(nrows,ncols)
...
#            else:
            xw = i + xn[n]
            yw = j + yn[n]
            tau = wd[i][j]*slope[i][j]/R/D50
            if tau > tau_star: #*R*D50:
#                AlSp = (wd[i][j]**2.0133)*(slope[i][j]**1.68)/mann[i][j]**2.0
                AlSp = (tau-tau_star)**1.68/(wd[i][j]**(-1.0/3)) /mann[i][j]**2.0
            else:
                AlSp = 0.0
    ...
    qs_downstream[tau>tau_star] = (tau[tau>tau_star]-tau_star)**(1.68)/(wd[tau>tau_star]**(-1.0/3))/(mann[tau>tau_star]**2.0)
#    qs_downstream = (wd**2.0133)*(slope**1.68)/mann**2.0
