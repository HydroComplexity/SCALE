...
def C_interpolate(Cl,Ch,Cb, z_matrix,z_matrix_update,dh_ST,nrows, ncols):
    ...
    # to maintain the top Cl, Ch, Cb do not change when interpolating
    pos_dh_ind = np.where(dh_ST>=0.0)
    z_matrix[0,pos_dh_ind] = z_matrix_update[0,pos_dh_ind]*1.0 - dh_ST[pos_dh_ind]
...
#    ratio_b = Cb/C
    ...
    for i in xrange(nrows*ncols):
        ...
        Ch_update[:,i] = interp1d((z_matrix[:,i]+dh_ST[i]), Ch[:,i],fill_value='extrapolate')(z_matrix_update[:,i]) # 
        Cb_update[:,i] = interp1d((z_matrix[:,i]+dh_ST[i]), Cb[:,i],fill_value='extrapolate')(z_matrix_update[:,i]) # 
...
def C_interpolate_simple(Cl,Ch,Cb, z_matrix,z_matrix_update,dh_ST,nrows, ncols):
    ...
    for i in xrange(nrows*ncols):
        ...
        C_update[:,i] = interp1d((z_matrix[:,i])+dh_ST[i], C[:,i],fill_value='extrapolate')(z_matrix_update[:,i]) # 
...
#    Kl_update = ratio_l*K_update
#    Kh_update = ratio_h*K_update
#    Kd_update = ratio_d*K_update
