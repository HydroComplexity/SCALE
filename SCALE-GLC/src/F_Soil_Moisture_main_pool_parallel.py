...
def f_SoilMoisture_parallel(params):
#        print k
...
#    Psi_n = Psi_n_all[:,k]
    ...
    k,theta_n,Psi_n,z,dz,wd_vector_cell,rainfall = params
    ...
    # Top Boundary Condition
    min_cap = np.min([Infil_max, (theta_S-theta_n[0])*dz[0]])
    
    if ((wd_vector_cell>0.0) and (rainfall>0.0) ):
        topbound = 1
        rain_depth = rainfall
        psi_upper = rainfall+wd_vector_cell
        q_upper= min_cap #0.0
    elif ((wd_vector_cell>0.0) and (rainfall==0.0) ):
        topbound = 1
        rain_depth = 0.0
        psi_upper = rainfall+wd_vector_cell-Evaps
        q_upper= min_cap-Evaps #0.0
    elif (rainfall>min_cap and wd_vector_cell==0.0 ) :
        topbound = 1
        rain_depth = rainfall-min_cap
        psi_upper =  rainfall-min_cap #  # Upper boundary
        q_upper = min_cap #0.0
    elif (rainfall<=min_cap and wd_vector_cell==0.0 and rainfall>0.0 ) :  
        topbound = 1
        rain_depth = 0.0
...
#    print multiprocessing.current_process() 
    ...
    return theta_n,Psi_n,rain_depth, Ts,Leak
#------------------------------------------------------------------------
# Initial Condition and grid size
#------------------------------------------------------------------------
def Soil_Moisture(rainfall, theta_n_all, Psi_n_all, z_matrix,dz_matrix,wd_vector,nrowsncols):

    z_matrix_T    =  np.transpose(z_matrix)
    dz_matrix_T   =  np.transpose(dz_matrix)
    theta_n_all_T =  np.transpose(theta_n_all)
    Psi_n_all_T   =  np.transpose(Psi_n_all)
    
    params = zip(np.arange(nrowsncols),theta_n_all_T,Psi_n_all_T,z_matrix_T,dz_matrix_T,wd_vector,rainfall*np.ones(nrowsncols))

    cores = multiprocessing.cpu_count()
    pool = multiprocessing.Pool(processes=cores)
    
    l = pool.map(f_SoilMoisture_parallel, params) 
    ...
    theta_n_all_new = np.asarray(l[0])
    Psi_n_all_new   = np.asarray(l[1])
    rain_depth      = np.asarray(l[2])
    Ts_n_all_new    = np.asarray(l[3])
    Leak_n_all_new  = np.asarray(l[4])
    
    theta_n_all_new = np.transpose(theta_n_all_new)
    Psi_n_all_new   = np.transpose(Psi_n_all_new)
    Ts_n_all_new    = np.transpose(Ts_n_all_new)
    Leak_n_all_new  = np.transpose(Leak_n_all_new)
...
#    rain = np.hstack([rain50,rain50[::-1]]) 
    ...
    for i in xrange(time_steps): 
        ...
        theta_n_all,Psi_n_all, rain_depth, Ts_n_all,Leak_n_all = \
            Soil_Moisture(rain[i+rp],theta_n_all, Psi_n_all, z_matrix,dz_matrix,rain_depth*0.0,nrows, ncols) #rain_depth
