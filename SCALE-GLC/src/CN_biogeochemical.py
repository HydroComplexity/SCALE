...
#    litter_input_unt  = (np.load('litter_input_untreated.npy'))/dt 
    ...
    if Carbon:
        litter_input       = litter_input_treC*1.0
    if Nature: 
        litter_input       = litter_input_tre*1.0
    litter_input_mean  = np.mean(litter_input,1) #[:,None]*np.ones((2,nrows*ncols))
...
#    theta_mean_1col = np.array([0.39126216, 0.39281667, 0.3923617,  0.39047139, 0.38715473, 0.38192914, 0.37363064])
    ...
    if Carbon:
        Cl_all_ini = 0.1235*(5300.0*np.exp(-2.0*z_matrix)+ 1.5)
        Ch_all_ini = 0.8752*(5300.0*np.exp(-2.0*z_matrix)+ 1.5)
        Cb_all_ini = 0.0104*(5300.0*np.exp(-2.0*z_matrix)+ 1.5)
    
        Cl_stable  = 0.1235*(5300.0*np.exp(-2.0*z_matrix)+ 1.5)*0.865 #48000
        Ch_stable  = 0.8752*(5300.0*np.exp(-2.0*z_matrix)+ 1.5)*0.865
        Cb_stable  = 0.0104*(5300.0*np.exp(-2.0*z_matrix)+ 1.5)*0.865
    
    if Nature:
        Cl_all_ini = 0.1235*(7600.0*np.exp(-2.0*z_matrix)+ 1.5)
        Ch_all_ini = 0.8752*(7600.0*np.exp(-2.0*z_matrix)+ 1.5)
        Cb_all_ini = 0.0104*(7600.0*np.exp(-2.0*z_matrix)+ 1.5)
    
        Cl_stable  = Cl_all_ini*1.0
        Ch_stable  = Ch_all_ini*1.0
        Cb_stable  = Cb_all_ini*1.0        
    ...
    Addb   = Frooti*(Add_b+litter_input_mean[1]) #+litter_mean[1]
    ADD    = Addb*1.0 
    ADD[0] = Addb[0]+ADD_s+litter_input_mean[0]  #+litter_mean[0]
    ...
    for ll in xrange(0,soil_layer_num):
        Kl[ll],Kh[ll],Kd[ll] = aveg_k(theta_mean_1col[ll]/poros,np.mean(Cl_stable[ll,:]), np.mean(Ch_stable[ll,:]), np.mean(Cb_stable[ll,:]),ADD[ll],rh[ll] )  
...
#        litter_below =(litter_input[1,i%(365*2)] + Add_b)*Frooti
        ...
        litter_above           = ADD_s + litter_input[0,i%(365)] #litter_input + ADD_s
        
        litter_below           = (litter_input[1,i%(365)] + Add_b)*Frooti
