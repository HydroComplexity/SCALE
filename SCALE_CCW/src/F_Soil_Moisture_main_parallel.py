...
#------------------------------------------------------------------------
def Soil_Moisture(nrows,ncols):#(rainfall, theta_n_all, Psi_n_all, z_matrix,dz_matrix,wd_vector,nrows,ncols):

    cores = multiprocessing.cpu_count()
    pool = multiprocessing.Pool(processes=cores)
    
    l = pool.map(f_grid_parallel, np.arange(nrows*ncols)) 
...
if __name__ == '__main__':
    #...................Rainfall
    rain = np.load('rainfall_100yr2_daily.npy') # mm/(day)
    rain = rain/1000.0
    nrows = 2
    ncols = 1
...
#    rain = np.hstack([rain50,rain50[::-1]]) 
    ...
    for i in xrange(time_steps): 
        print i
        wd_vector = rain_depth*0.0 
        rainfall = rain[i+rp]
        theta_n_all,Psi_n_all, rain_depth, Ts_n_all,Leak_n_all = \
            Soil_Moisture(nrows, ncols) #(rainfall,theta_n_all, Psi_n_all, z_matrix,dz_matrix,wd_vector,1,1) #rain_depth
